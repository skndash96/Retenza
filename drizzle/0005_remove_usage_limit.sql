-- Remove usage_limit field from limited usage rewards and ensure usage_limit_per_month is set
-- This migration updates existing loyalty program tiers to remove usage_limit and set default usage_limit_per_month

-- Create a function to update existing JSONB data
CREATE OR REPLACE FUNCTION remove_usage_limit_field()
RETURNS void AS $$
DECLARE
                ...
                IF tier_record->'rewards' IS NOT NULL AND jsonb_array_length(tier_record->'rewards') > 0 THEN
                    FOR j IN 0..jsonb_array_length(tier_record->'rewards') - 1 LOOP
                        ...
                        IF reward_record->>'reward_type' = 'limited_usage' THEN
                            -- For limited_usage rewards, remove usage_limit and ensure usage_limit_per_month exists
                            updated_rewards := updated_rewards || jsonb_build_object(
                                ...
                                'usage_limit_per_month', COALESCE(reward_record->'usage_limit_per_month', 1.0),
...
SELECT remove_usage_limit_field();

-- Drop the function after use
DROP FUNCTION remove_usage_limit_field();

-- Add comment to explain the updated structure
COMMENT ON COLUMN loyalty_programs.tiers IS 'JSONB array of tiers with rewards. Limited usage rewards use only usage_limit_per_month (no separate usage_limit field).';